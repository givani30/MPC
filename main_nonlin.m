...
nlobj=nlmpc(6,4,3);
nlobj.Model.StateFcn=@LTIC;
nlobj.Jacobian.StateFcn=@jacobian;
nlobj.Ts=Ts;
nlobj.Model.OutputFcn=@outputfun;
nlobj.PredictionHorizon = 2/Ts; %2 sec
nlobj.ControlHorizon=5; %0.1 Sec
...
nlobj.Weights.ManipulatedVariables=[1 5 5];

%Constraints


nlobj.ManipulatedVariables(2).RateMax=5*Ts; %Max throttle rate of change
nlobj.ManipulatedVariables(3).RateMax=5*Ts; %Max throttle rate of change

nlobj.ManipulatedVariables(2).RateMin=-5*Ts; %Min throttle rate of change
nlobj.ManipulatedVariables(3).RateMin=-5*Ts; %Min throttle rate of change

% % #Constraint on turing radius of car
% max_angle=1.5; %Max steering angle
nlobj.MV(1).RateMax=pi/30*Ts;
nlobj.MV(1).RateMin=-pi/30*Ts;
...
% % 
% mpcobj.ManipulatedVariables(2).Min=0; %Min throttle (brake) [N*m]
% mpcobj.ManipulatedVariables(3).Min=0; %Min throttle (brake) [N*m]
% % mpcobj.MV(2).MinECR=0.1;
% % mpcobj.MV(3).MinECR=0.1;
% % %TODO scale MV
...
nlobj.OV(1).ScaleFactor=1/100;
nlobj.OV(2).ScaleFactor=1/100;
nlobj.OV(3).ScaleFactor=1/200;
nlobj.OV(4).ScaleFactor=1/100;
% 
% mpcobj.ManipulatedVariables(1).ScaleFactor=max_angle; %Scale steering angle
% %% 
% mpcobj.Weights.ECR=1e4;
% % # Weights on output vars
% mpcobj.Weights.ManipulatedVariables=[1 10 10];
% mpcobj.Weights.OutputVariables=[10000 100 30000 1e-9]; %Weight on x_dot,y_dot,psi and y
...
% setconstraint(nlobj, E,F,G,[0.1;0.1;0.01]);
...
x=eps_0;
u=u_0;
y=outputfun(x,u,{parameters});
T=0:Ts:10;
measurementfun=@(y)outputfun(y,u,parameters);
EKF=extendedKalmanFilter(@LTIC,measurementfun,eps_0);
...
validateFcns(nlobj,eps_0,u_0,[],{parameters})
for i=1:length(T)
    %Detection logic
...
%     opt.CustomConstraint=struct('E',E,'F',F,'G',G);
    ...
    [u,nloptions,info]=nlmpcmove(nlobj, xk, u, [], [],opt);

    %Time update of the system
%     x=x+Ts*LTIC(x,u,parameters);
    predict(EKF,u,parameters);
    x=LTID(x,u,parameters,Ts);
    %Save the results
    states(:,i)=x;
    inputs(:,i)=u;
end
